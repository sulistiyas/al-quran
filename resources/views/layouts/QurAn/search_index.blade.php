                                ...
                                <div class="timeline">
                                    ...
                                    <div class="">
                                        ...
                                        @foreach ($history_data as $item_history)
                                                <div class="timeline-item" class="form-group">
                                                    <span class="time"><i class="fas fa-clock"></i> {{ $item_history->created_at }}</span>
                                                    <h3 class="timeline-header">
                                                        <form action="{{ route('delete_history',[$item_history->id_search_history]) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="{{ route('history_result',[$item_history->keyword]) }}">{{ $item_history->keyword }}</a>
                                                            <button style="background-color: transparent; border: none"><i class="fas fa-times-circle" style="color:red"></i></button>
                                                            {{-- <a href="{{ route('delete_history',[$item_history->id_search_history]) }}"><i class="fas fa-times-circle" style="color:red"></i></a> --}}
                                                        </form>
                                                    </h3>
                                                </div>
                                        @endforeach
